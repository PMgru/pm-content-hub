...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_id'])) {
        ...
        if ($rating >= 1 && $rating <= 5) {
            $stmt = $conn->prepare("UPDATE posts SET rating = ? WHERE id = ?");
            $stmt->execute([$rating, $post_id]);
...
<head>
    ...
    <title><?php echo htmlspecialchars($post['title']); ?> - Content Learning Hub</title>
...
<body>
    ...
    <section class="post-details">
        ...
        <div class="rating">
            ...
            <?php endfor; ?>
            <span class="rating-count"><?php echo htmlspecialchars($post['rating']); ?>/5</span>
        </div>
        ...
        <?php else: ?>
            <p>Please <a href="login.php">login</a> to rate this post.</p>
        <?php endif; ?>
    </section>
